...
async function createTerritoryPartnerApplication(data) {
  ...
  // $15 territory_zips
  // $16 territory_cities
  // $17 territory_counties
  // $18 territory_radius_center
  // $19 territory_radius_miles
  // $20 territory_notes
  ...
  `, [
    ...
    data.zip,                                   // $8
    data.website || null,                       // $9
    data.businessDescription || null,           // $10
    data.tier,                                  // $11
    JSON.stringify(data.industries),            // $12
    data.territoryName || null,                 // $13
    data.territoryType || null,                 // $14
    data.territoryZips ? JSON.stringify(data.territoryZips) : null,  // $15
    data.territoryCities || null,               // $16
    data.territoryCounties || null,             // $17
    data.territoryRadiusCenter || null,         // $18
    data.territoryRadiusMiles || null,          // $19
    data.territoryNotes || null,                // $20
  ]);
...
async function savePartnerCredentials(id, hubLoginId, hubPassword, hubToken) {
  const result = await pool.query(`
    UPDATE territory_partner_applications
    SET hub_login_id = $1, hub_password = $2, hub_token = $3
    WHERE id = $4
    RETURNING *
...
async function getPartnerByHubToken(token) {
  const result = await pool.query(`
    SELECT * FROM territory_partner_applications
    WHERE hub_token = $1 AND status = 'approved'
  `, [token]);
  return result.rows[0] || null;
}

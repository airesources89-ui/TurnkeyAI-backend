  ...
  try {
    ...
    if (typeof content === 'string') {
      ...
      Object.keys(content).forEach(name => {
        // Support subdirectory paths like 'blog/index' or 'blog/post-1'
        const filename = name === 'index' ? 'index.html' : name + '.html';
        const fullPath = path.join(tmpDir, filename);
        const dir = path.dirname(fullPath);
        if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
        fs.writeFileSync(fullPath, content[name], 'utf8');
      });
    }
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  <url><loc>${siteBaseUrl}/blog/index.html</loc><changefreq>weekly</changefreq><priority>0.7</priority></url>
</urlset>`;
...
async function runDeploy(client) {
  ...
  // ── Blog generation: fire-and-forget after deploy ──
  // Runs async so it never delays the approval response to the user.
  // Only fires for plans that include blog.
  (() => {
    const { generateAndDeployBlog, planIncludesBlog } = require('./blog-generator');
    if (!planIncludesBlog(client)) {
      console.log(`[blog-generator] Skipping blog for ${client.data.businessName} (plan: ${client.data.selectedPlan || 'none'})`);
      return;
    }
    console.log(`[blog-generator] Triggering 30-post blog generation for ${client.data.businessName}...`);
    generateAndDeployBlog(client, 30).catch(err =>
      console.error(`[blog-generator] Blog generation failed for ${client.data.businessName}:`, err.message)
    );
  })();

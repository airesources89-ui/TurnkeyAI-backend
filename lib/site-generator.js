...
<section style="padding:4rem 1.5rem;background:white;text-align:center;">
  ...
  const schedulingExtraCSS = `
    .page-hero{background:linear-gradient(135deg,${pal.primary} 0%,#1a3a6b 100%);padding:8rem 1.5rem 4rem;text-align:center}
    .booking-section{background:#f0f4ff}
    .booking-layout{display:grid;grid-template-columns:1fr 1fr;gap:2.5rem;margin-top:2.8rem;align-items:start}
    .booking-form{background:white;border-radius:16px;padding:2rem;box-shadow:0 8px 32px rgba(10,22,40,.1)}
    .cal-strip{display:flex;gap:8px;overflow-x:auto;padding:4px 0 12px;margin-bottom:1.2rem}
    .cal-day{min-width:72px;padding:10px 8px;border-radius:10px;text-align:center;cursor:pointer;border:2px solid #e2e8f0;background:white;transition:all .2s;flex-shrink:0}
    .cal-day:hover{border-color:${pal.accent};background:#fffcf5}
    .cal-day.active{border-color:${pal.accent};background:${pal.accent};color:${pal.primary}}
    .cal-day.active .cal-dayname,.cal-day.active .cal-date{color:${pal.primary}}
    .cal-day.closed{opacity:.4;cursor:not-allowed;background:#f1f5f9}
    .cal-dayname{font-size:.68rem;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:#64748b;margin-bottom:2px}
    .cal-date{font-size:1.1rem;font-weight:700;color:#0a1628}
    .cal-month{font-size:.62rem;color:#94a3b8;margin-top:1px}
    .slots-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(105px,1fr));gap:8px;margin-bottom:1.5rem}
    .slot{padding:10px 6px;border-radius:8px;text-align:center;font-size:.85rem;font-weight:600;cursor:pointer;border:2px solid #e2e8f0;background:white;transition:all .2s}
    .slot.available:hover{border-color:${pal.accent};background:#fffcf5;transform:translateY(-1px)}
    .slot.available.selected{border-color:${pal.accent};background:${pal.accent};color:${pal.primary}}
    .slot.booked{background:#fee2e2;border-color:#fecaca;color:#dc2626;cursor:not-allowed;text-decoration:line-through;opacity:.6}
    .slot-status{font-size:.62rem;display:block;margin-top:2px;font-weight:400}
    .slots-loading{text-align:center;padding:2rem;color:#94a3b8;font-size:.9rem}
    .slots-closed{text-align:center;padding:2rem;color:#dc2626;font-size:.9rem;background:#fef2f2;border-radius:10px}
    @media(max-width:768px){.booking-layout{grid-template-columns:1fr}.cal-strip{gap:6px}}`;
...
<section class="page-hero">
  <div class="container">
    ...
    <p style="color:rgba(255,255,255,.75);font-size:1.05rem;max-width:520px;margin:1rem auto 0;line-height:1.7;">Pick a date and time \u2014 you'll get instant confirmation.</p>
  </div>
</section>
...
<section class="booking-section">
  <div class="container">
    <div class="booking-layout">
      <div>
        <div class="reveal">
          <div class="section-label">1. Choose a Date</div>
          <h3 style="font-size:1.1rem;font-weight:700;color:#0a1628;margin-bottom:1rem;">Select a Day</h3>
        </div>
        <div class="cal-strip" id="calStrip"></div>
        <div class="reveal" style="margin-top:.5rem;">
          <div class="section-label">2. Pick a Time</div>
          <h3 style="font-size:1.1rem;font-weight:700;color:#0a1628;margin-bottom:1rem;">Available Time Slots</h3>
        </div>
        <div id="slotsArea"><div class="slots-loading"><i class="fas fa-arrow-up"></i> Select a date above to see available times</div></div>
      ...
      <div class="booking-form reveal">
        <h4 style="font-size:1.05rem;font-weight:700;color:#0a1628;margin-bottom:.4rem;"><i class="fas fa-calendar-check" style="color:${pal.accent};margin-right:.4rem;"></i> 3. Your Information</h4>
        <div id="selectedSummary" style="background:#f0f9ff;border:2px solid ${pal.accent};border-radius:10px;padding:12px 16px;margin-bottom:1.2rem;display:none;">
          <p style="margin:0;font-size:.85rem;color:#1e40af;"><strong>Selected:</strong> <span id="summaryText"></span></p>
        </div>
        <div class="form-row">
        ...
        <button class="btn-book" id="bookBtn" onclick="submitBooking(this)" disabled style="opacity:.5;cursor:not-allowed;"><i class="fas fa-calendar-check"></i> Select a Date &amp; Time First</button>
        <p style="font-size:.72rem;color:#64748b;text-align:center;margin-top:.65rem;">You'll receive instant confirmation by email and text.</p>
      </div>
    </div>
  </div>
</section>
...
(function(){
  ...
  function buildCalStrip(){
    ...
    for(var i=0;i<14;i++){
      ...
      html+='<div class="cal-day" data-date="'+dateStr+'" onclick="selectCalDay(this)">';
      html+='<div class="cal-dayname">'+dayNames[d.getDay()]+'</div>';
      html+='<div class="cal-date">'+d.getDate()+'</div>';
      html+='<div class="cal-month">'+monthNames[d.getMonth()]+'</div>';
      html+='</div>';
    }
    strip.innerHTML=html;
  }

  window.selectCalDay=function(el){
  ...
  function loadSlots(dateStr){
    ...
    if(!CLIENT_ID){
      area.innerHTML='<div class="slots-loading">Scheduling will be available when this site goes live.</div>';
      return;
    }
    area.innerHTML='<div class="slots-loading"><i class="fas fa-spinner fa-spin"></i> Loading available times\u2026</div>';
    fetch(API+'/api/appointments/available/'+CLIENT_ID+'?date='+dateStr)
      .then(function(r){return r.json();})
      .then(function(data){
        if(data.closed){
          area.innerHTML='<div class="slots-closed"><i class="fas fa-store-slash"></i> Closed on '+capitalize(data.dayName||'this day')+'</div>';
          return;
        }
        if(!data.slots||!data.slots.length){
          area.innerHTML='<div class="slots-closed">No available time slots for this date.</div>';
          return;
        }
        var html='<div class="slots-grid">';
        data.slots.forEach(function(s){
          if(s.available){
            html+='<div class="slot available" onclick="selectSlot(this,\\''+s.time+'\\')">';
            html+=s.time+'<span class="slot-status" style="color:#16a34a;">Available</span></div>';
          }else{
            html+='<div class="slot booked">';
            html+=s.time+'<span class="slot-status">Booked</span></div>';
          }
        });
        html+='</div>';
        area.innerHTML=html;
      ...
      .catch(function(){
        area.innerHTML='<div class="slots-closed">Unable to load times. Please try again or call us.</div>';
      });
  }

  window.selectSlot=function(el,time){
    document.querySelectorAll('.slot.available').forEach(function(s){s.classList.remove('selected');});
  ...
  function updateSummary(){
    ...
    if(selectedDate&&selectedTime){
      ...
      text.textContent=d.toLocaleDateString('en-US',opts)+' at '+selectedTime;
      summary.style.display='block';
      btn.disabled=false;
      btn.style.opacity='1';
      btn.style.cursor='pointer';
      btn.innerHTML='<i class="fas fa-calendar-check"></i> Book '+selectedTime+' Appointment';
    }else if(selectedDate){
      text.textContent='Pick a time slot above';
      summary.style.display='block';
      btn.disabled=true;
      btn.style.opacity='.5';
      btn.style.cursor='not-allowed';
      btn.innerHTML='<i class="fas fa-calendar-check"></i> Select a Time Slot';
    }else{
      summary.style.display='none';
      btn.disabled=true;
      btn.style.opacity='.5';
      btn.style.cursor='not-allowed';
      btn.innerHTML='<i class="fas fa-calendar-check"></i> Select a Date & Time First';
    }
  }
  ...
  window.submitBooking=function(btn){
    ...
    if(!ph&&!em){alert('Please enter a phone number or email so we can confirm.');return;}
    btn.innerHTML='<i class="fas fa-spinner fa-spin"></i> Booking\u2026';btn.disabled=true;
    ...
    .then(function(res){
      if(!res.ok){
        if(res.data.error&&res.data.error.indexOf('just booked')>-1){
          ...
          btn.innerHTML='<i class="fas fa-calendar-check"></i> Select a Time Slot';btn.disabled=true;btn.style.opacity='.5';
        }else{
          alert(res.data.error||'Booking failed. Please try again.');
          btn.innerHTML='<i class="fas fa-calendar-check"></i> Book '+selectedTime+' Appointment';btn.disabled=false;btn.style.opacity='1';
        }
        return;
      }
      btn.innerHTML='<i class="fas fa-check-circle"></i> Booked!';btn.style.background='#16a34a';btn.style.color='white';
      var d=new Date(selectedDate+'T12:00:00');
      var nice=d.toLocaleDateString('en-US',{weekday:'long',month:'long',day:'numeric'});
      document.getElementById('slotsArea').innerHTML='<div style="text-align:center;padding:2rem;background:#f0fff4;border-radius:12px;border:2px solid #00D68F;"><h3 style="color:#065f46;margin:0 0 8px;">\u2705 You\\'re Booked!</h3><p style="color:#374151;margin:0;font-size:1rem;"><strong>'+nice+'</strong> at <strong>'+selectedTime+'</strong></p><p style="color:#64748b;font-size:.85rem;margin:8px 0 0;">Check your email and phone for confirmation.</p></div>';
      document.getElementById('selectedSummary').innerHTML='<p style="margin:0;color:#065f46;font-weight:700;">\u2705 Appointment confirmed!</p>';
    })
    .catch(function(){
      btn.innerHTML='<i class="fas fa-calendar-check"></i> Book '+selectedTime+' Appointment';btn.disabled=false;btn.style.opacity='1';
